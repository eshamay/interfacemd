...
void PDBFile::LoadNext () {
	...
	_molecules.clear();		// first empty out the list of molecules in the system
	_numAtoms = 0;
	_numMols = 0;
...
void PDBFile::LoadFirst() {
	...
	_atoms.clear();
	RUN (_molecules) {
		for (int atom = 0; atom < _molecules[i]->size(); atom++) {
			_atoms.push_back (_molecules[i]->Atoms(atom));
		}
	}
}
