...
namespace morita {
	...
	const  double M		=	(sfg_units::MOXY*sfg_units::MHYD)/(sfg_units::MOXY+sfg_units::MHYD);	// reduced mass of the OH bond/oscillator
	...
	const double UNCOUPLED_OH_FREQ	= 3706.5/sfg_units::AU2WAVENUMBER;			// the frequency of uncoupled OH bonds in the vapor phase (converted to frequency in atomic units)
	const double COUPLING_CONST		= 49.5/sfg_units::AU2WAVENUMBER;				// Coupling const taken from the energy gap of the sym + antisym stretches (V12 in atomic units)
	...
	const double OH_LENGTH = 1.000*sfg_units::ANG2BOHR;	// in atomic units
	const double OH_COM_LENGTH = sfg_units::MHYD*OH_LENGTH/(sfg_units::MHYD+sfg_units::MOXY);	// distance to the center of mass of the OH bond from the oxygen (atomic units of length)
	...
	const  double GAMMA				= 2.0/sfg_units::AU2WAVENUMBER;				// the "damping parameter" first shown in Eq. 3 scaled down to some value (between 2 and 22 cm-1 as Dave put it)
	const  double GAMMA_SQ			= GAMMA*GAMMA;				// ...squared

	// frequency limits for calculating the spectra (given in cm-1)
	const  double START_FREQ			= 2800.0/sfg_units::AU2WAVENUMBER;
	const  double END_FREQ			= 3800.0/sfg_units::AU2WAVENUMBER;
	const  double FREQ_STEP			= 1.0/sfg_units::AU2WAVENUMBER;		// step size when calculating the spectra
	...
	class SFGCalculator {
		...
		private:
			...
			Complex_vec _Beta;		// hyperpolarizability of a given water (in the molecular frame)
			Complex_vec _Chi;		// hyperpolarizability of a given water (in the molecular frame)
			...
			void DipoleDipoleContribution (morita::MoritaH2O& water, std::vector<double>& dipolePotential);
#endif

			double CouplingConstant (morita::MoritaH2O& water) const;


			void FreqShift (morita::MoritaH2O& water);		// Calculate the frequency shift on a given water-OH bond

			void WaterEigenSystem (morita::MoritaH2O& water);

			// calculate the total value of the collective product of the dipole and polarizability derivative terms
			void PolarizabilityAndDipoleDerivs (morita::MoritaH2O& water, const int s1, const int s2, const int p);
		...
		public:
			...
			std::vector< std::complex<double> >& Beta (morita::MoritaH2O& water, const int s1, const int s2, const int p);
	};
